...
namespace Logistics.DbMerger
{
    public class DataMigrator
    {
        ...
        public async Task MigrateTableAsync(string sourceTableName, bool isNewTable, string targetTableName = null, int? sourceTenantId = null, int? targetTenantId = null, Dictionary<long, long> userMapping = null)
        {
    ...
    var targetSchema = await targetConn.GetSchemaAsync("Columns", new[] { null, null, destTable });
            ...
            // Check if we need to transform TenantId OR UserIds
                            ...
                            foreach(DataRow row in dt.Rows)
                            {
                                if (row[colName] != DBNull.Value)
                                {
                                    try
                                    {
                                        ...
                                        if (userMapping.ContainsKey(oldId))
                                        {
                                            row[colName] = userMapping[oldId];
                                        }
                                    }
